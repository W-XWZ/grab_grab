function matchConditionByVariety(current_priority, sheetItem){
	...
	//类别
	let itemId = sheetItem['itemId'] == null ? "":sheetItem['itemId'];
	...
	if (current_priority['itemId'] != '' && !!current_priority['itemId']) {
		let itemId_match = false;
		let itemId_array = current_priority['itemId'].split(",");
		for (let itemId_index = 0; itemId_index < itemId_array.length; itemId_index++) {
			let itemId_compare = itemId_array[itemId_index].trim();
			if (itemId == itemId_compare) {
				itemId_match = true;
				break;
			}
		}
...
function loadupDisplayCondition(conditionPriorItem, index) {
	...
	let itemId = conditionPriorItem["itemId"];
	match += "仓库: " + warehouse + "等级: " + level + "类别:" + itemId;
	return match;
}
...
function validateCondition(editPriorNum){
	for (let i = 1; i <= editPriorNum; i++) {
		...
		let itemId = $('#itemId_' + i).val();
		if(validateItemId(itemId) == false){
		    alert("参数错误， itemId：" + itemId);
		    return;
		}
	}
}
...
function conditionPanelInnerHtml(conditionPriorItem, index){
	let innerHTML = `优先度` + index + `:
				...
				<input class = "condition-input" id="level_` +
					...
					`"/> 
				<input class = "condition-input" id="itemId_` +
					index + `" placeholder="类别" type="text" style='width:100px' value = "` + conditionPriorItem['itemId'] +
					`"/> </br>
					`;
	return innerHTML;
}
...
function addConditionPanelHtml(editPriorNum){
	let innerHTML = `
			...
			<input class = "condition-input" id="warehouse_` + editPriorNum +
			...
			<input class = "condition-input" id="itemId_` +
			editPriorNum +
			`" placeholder="类别" type="text" style='width:100px'/>
		</br>
		`;
	return innerHTML;
}
...
function getParam(variety, commodity_id, round_number){
	...
	param.formDefine=JSON.parse(`{"formDefineId":10032,"formId":"selection_query_condition","formAttrDefines":[{"attrId":13044,"formAttrDefineId":4054,"name":"WH_ID","bizName":"whId","value":null},{"attrId":13043,"formAttrDefineId":4055,"name":"MAKE_DATE","bizName":"makeDate","value":null},{"attrId":13041,"formAttrDefineId":4056,"name":"PLACE_NAME","bizName":"placeName","value":null},{"attrId":13039,"formAttrDefineId":4057,"name":"LEV_ID","bizName":"levId","value":null}]}`);
	return param;
}
...
//保存输入的条件
	...
	function displayWareHouseValue(item, id, result){
		...
		let balQty = item['balQty'];
		let itemId = item['itemId'];
		let priority_num = item['priority_num'];
		let innerHTML =  `
				...
				<td class = "cus-td">` + balQty + `</td>
				<td class = "cus-td">` + itemId + `</td>
				<td class = "cus-td">` + priority_num + `</td>
