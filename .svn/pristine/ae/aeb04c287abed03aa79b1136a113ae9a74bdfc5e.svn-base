...
//保存输入的条件
	...
	function displayWareHouseValue(item, id, result){
		...
		let totalSulfur = null == item['totalSulfur'] ? '-' : item['totalSulfur'];
		let netCalorific = null == item['netCalorific'] ? '-' : item['netCalorific'];
		
		let coleType = null == item['coleType'] ? '-' : item['coleType'];
		...
		let innerHTML =  `
				...
				<td class = "cus-td">` + netCalorific + `</td>
				<td class = "cus-td">` + totalSulfur + `</td>
				<td class = "cus-td">` + placeName + `</td>
				<td class = "cus-td">` + coleType + `</td>
